...
class Bin:
    ...
    def __init__(self, partno, WHD, max_weight,corner=0,put_type=1):
        ...
        self.gravity = []
        self.cushioning_materials_list = []  # New attribute to store items and cushioning materials

    def cushion_needed(self, item, unsupported_ratio):
        ''' Calculate how much weight the cushioning material needs to support and choose the right material. '''
        
        # Define cushioning materials with their weight capacities (in grams)
        cushioning_materials = {
            "Foam bag": 5,  # Supports up to 5000 grams
            "Foam board": 10,  # Supports up to 10000 grams
            "Cardboard": 20,  # Supports up to 2000 grams
        }
        
        # Calculate the item's base area (width * depth) in square units
        base_area = Decimal(item.width) * Decimal(item.depth)
        
        # Calculate the weight per square unit of the item's base
        weight_per_square_unit = Decimal(item.weight) / base_area
        
        # Convert unsupported_ratio to Decimal before multiplication
        unsupported_area = base_area * Decimal(unsupported_ratio)
        
        # Calculate the weight that needs to be supported by the cushioning material
        weight_to_support = unsupported_area * weight_per_square_unit
        
        # Print out the unsupported area and weight to support for debugging or output purposes
        print(f"Item {item.partno} has {unsupported_area:.2f} square units unsupported, requiring {weight_to_support:.2f} Kg of support.")
        
        # Check which cushioning material can support the required weight
        for material, max_weight in cushioning_materials.items():
            if weight_to_support <= Decimal(max_weight):
                print(f"Item {item.partno} needs {material} as cushioning material to support {weight_to_support:.2f} Kg.")
                # Append item name and the cushioning material to the list in the Bin class
                self.cushioning_materials_list.append((item.partno, material))
                return True  # Allow item to stay with this cushioning material
        
        # If no material can support the required weight, remove the item
        print(f"Item {item.partno} is too heavy and cannot be placed with available cushioning materials. It requires {weight_to_support:.2f} Kg of support.")
        self.unfitted_items.append(item)  # Remove the item
        return False
                ...
                if self.fix_point == True :
                    ...
                    if self.check_stable == True :
                        ...
                        if support_area_upper / item_area_lower < self.support_surface_ratio :
                            unsupported_ratio = 1 - (support_area_upper / item_area_lower)
                            if not self.cushion_needed(item, unsupported_ratio):
                                # If cushioning is not enough, the item is unfitted
...
class Packer:
    ...
    def gravityCenter(self,bin):
        ...
        # Check if total_weight is zero before dividing
        if sum(r) == 0:
            result = [0, 0, 0, 0]  # No items in the bin, no gravity distribution
            return result
        else:
            for i in r :
